...
#if defined(NDEBUG)
# error "Socialcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x716b50a3d5c5e9c5cd4fdbdf68c618712e3595cc324053ffd617082103cf5e6b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Socialcoin: starting difficulty is 1 / 2^12
...
{
    // Socialcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 495000 blocks
    nSubsidy >>= (nHeight / 495000);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Socialcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Socialcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1497892811, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "London fire Lives claimed at Grenfell Tower";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 75 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0474a2b7b3241be735759a1713f20c7caec462225cf2b8b6a51d29ada5cc57622e2735cff941fc5485a5f99d941595ffc373a2a48385950a66b4b8e0ef5a64db3d") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1497892811;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1968465;
        ...
        assert(block.hashMerkleRoot == uint256("0x4351eae3b727544bab064d89adc3e9306948134f2b7ddd65644d97a09ee2faef"));
...
unsigned char pchMessageStart[4] = { 0xee, 0x64, 0xe3, 0x1d }; // Socialcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SocialcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SocialcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SocialcoinMiner(CWallet *pwallet)
{
    printf("SocialcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("socialcoin-miner");
    ...
    try { loop {
        ...
        printf("Running SocialcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SocialcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SocialcoinMiner, pwallet));
}
